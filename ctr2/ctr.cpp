...
void c_ctr::read_init_information(const char* theta_v_init_path,
                                  const char* beta_init_path,
                                  const c_corpus* c,
                                  double alpha_v_smooth) {
  int num_topics = m_num_factors;
  m_theta_v = gsl_matrix_alloc(c->m_num_docs, num_topics);
...
void c_ctr::read_init_information_lib(const char* theta_u_init_path,
                                  const c_corpus* c_lib,
                                  double alpha_u_smooth) {
  int num_topics = m_num_factors;
  m_theta_u = gsl_matrix_alloc(c_lib->m_num_docs, num_topics);
  printf("\nreading theta_lib initialization from %s\n", theta_u_init_path);
  ...
  while (iter < param->max_iter) {
      ...
      if (n > 0) {
        ...
        while (true) {
          ...
          if (positive) {
            ...
            theta_v = gsl_matrix_row(m_theta_v, j);
            // second-order
            // u
            ...
            theta_v = gsl_matrix_row(m_theta_v, jj);
            // second order
            // u
            gsl_vector_scale(&u.vector, 1 - learning_rate);
    ...
    if (iter % param->save_lag == 0) {
      ...
      if (param->ctr_run && param->theta_v_opt) {
        ...
        gsl_matrix_fwrite(file_theta, m_theta_v);
        fclose(file_theta);
...
void c_ctr::learn_map_estimate(const c_data* users, const c_data* items, 
  ...
  while ((iter < param->max_iter and converge > 1e-4 ) or iter < min_iter) {
    ...
    // update V
    if (param->ctr_run && param->theta_v_opt) gsl_matrix_set_zero(word_ss);
    ...
    if (iter % param->save_lag == 0) {
      ...
      if (param->ctr_run) { 
        sprintf(name, "%s/%04d-theta.dat", directory, iter);
        FILE * file_theta = fopen(name, "w");
        mtx_fprintf(file_theta, m_theta_v);
        fclose(file_theta);
