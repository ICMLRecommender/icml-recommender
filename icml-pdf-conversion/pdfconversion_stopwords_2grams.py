...
##    data, vocab=cleanVocabulary(vectorizer.get_feature_names(),data)
    out_mult=codecs.open(path.join(file_dir,"mult_stop.dat"),mode="w",encoding="utf-8",errors="ignore")
    ...
    out_vocab=codecs.open(path.join(file_dir,"vocab_stop.dat"),mode="w",encoding="utf-8",errors="ignore")
    for key,index in sorted(vocab.items(), key=lambda x:x[1]):
        out_vocab.write(key+"\n")
    out_vocab.close()

    out_files=codecs.open(path.join(file_dir,"files_stop.dat"),mode="w",encoding="utf-8",errors="ignore")
